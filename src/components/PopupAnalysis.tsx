...
export default function PopupAnalysis() {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-3 sm:p-6 overflow-hidden">
            <div className="bg-[#e9e9e9] border border-brand-black/5 rounded-[1.5rem] sm:rounded-[2rem] w-full max-w-2xl relative shadow-2xl p-5 sm:p-8 overflow-y-auto max-h-full scrollbar-hide">
                
                {/* Close Button */}
                <button 
                    onClick={() => setIsOpen(false)}
                    className="absolute top-3 right-3 sm:top-4 sm:right-4 w-8 h-8 flex items-center justify-center rounded-full bg-black/5 hover:bg-black/10 transition-colors z-10"
                >
                ...
                <form onSubmit={handleSubmit} className="space-y-2.5 sm:space-y-5">
                    ...
                    <div className="grid grid-cols-1 gap-2.5 sm:gap-5">
                        <div className="space-y-1 sm:space-y-1.5">
                            ...
                            <input
                                ...
                                id="popup-fullName"
                                required
                                className="w-full bg-white border border-brand-black/10 rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 outline-none focus:border-brand-red/30 focus:ring-4 focus:ring-brand-red/5 transition-all text-base text-brand-black"
                    ...
                    <div>
                        <label className="text-xs sm:text-sm font-bold text-brand-black/70 ml-1 block mb-1.5 sm:mb-3">How soon are you looking to start the ranking process? *</label>
                        <div className="flex flex-wrap gap-2 sm:gap-3">
                            {["Today", "Tomorrow", "A few weeks"].map((option) => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 sm:px-5 sm:py-2.5 rounded-full text-xs sm:text-sm font-medium transition-all duration-200 border ${formData.urgency === option
                    ...
                    <div className="pt-2 sm:pt-2">
                        <LiquidButton
                            className="w-full py-3 sm:py-4 text-base sm:text-lg font-bold shadow-xl hover:shadow-2xl hover:shadow-brand-red/20 transition-all opacity-100 disabled:opacity-70 disabled:cursor-not-allowed"
                            disabled={loading}
                        >
